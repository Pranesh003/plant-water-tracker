...
export default function Analytics() {
  ...
  return (
    <>
      <section className="page-title"><p className="eyebrow">Plant insights</p><h1>Analytics</h1></section>
      ...
      <section className="insight-grid"><div className="panel top-plant"><Award size={34} /><p>Most Consistent Plant</p><h2>{analytics.topPlant?.icon} {analytics.topPlant?.name}</h2><strong>{analytics.topPlant?.consistency}% watering consistency</strong></div><div className="panel leaderboard"><h2><Flame size={19} /> Streak Leaderboard</h2>{paginatedLeaderboard.map((plant, index) => <div key={plant.id}><span>{((leaderboardPage - 1) * PAGE_SIZE) + index + 1}. {plant.icon} {plant.name}</span><strong>{plant.currentStreak} days</strong></div>)}<Pagination page={leaderboardPage} totalItems={leaderboard.length} pageSize={PAGE_SIZE} onPageChange={setLeaderboardPage} /></div><div className="panel leaderboard"><h2><Leaf size={19} /> Consistency</h2><p className="large-percent">{analytics.consistency}%</p><small>Completed on time across your active plant family.</small></div></section>
    </>
  );
}
